...
export interface ClaudePlan {
  ...
  arrangement: { deck: DeckId; srcBar: number; lengthBars: number; startBar: number; lane: number; label: string; stem: StemKey; mode: "layer" | "swap"; exact?: { startBeat: number; lengthBeats: number; fadeIn: number; fadeOut: number; gain?: number; eq?: { low: number; mid: number; high: number }; fx?: { echo?: number; reverse?: boolean }; semitones?: number } }[];
  tips: string[];
...
export const useStore = create<Store>((set, get) => {
  ...
  const candidateToPlan = (c: PlanCandidate, labels?: string[]): ClaudePlan => ({
    ...
    arrangement: c.clips.map((k, i) => ({
      ...
      stem: k.stem,
      mode: k.mode,
      exact: { startBeat: k.startBeat, lengthBeats: k.lengthBeats, fadeIn: k.fadeIn, fadeOut: k.fadeOut, ...(k.gain !== undefined ? { gain: k.gain } : {}), ...(k.eq ? { eq: k.eq } : {}), ...(k.fx ? { fx: k.fx } : {}), ...(k.semitones !== undefined ? { semitones: k.semitones } : {}) },
    })),
    tips: [],
  });
