...
- **Song structure** – each song gets intro / verse / chorus / bridge / break / outro blocks above its waveform from a self-similarity analysis, with the strongest chorus marked as the hook. Click a block to select it, drag it straight onto the timeline, drag its edges to move boundaries, double-click to relabel, right-click to split or merge. Edits are saved with the song and respected by the planner; "Re-detect sections" in the grid panel starts over, and "Re-analyse" runs tempo, downbeat and key detection again (songs analysed by an older version keep their old reading until you do; ÷2 and ×2 fix an octave error in one click and are saved with the song). The foundation block on the timeline shows the same labels. Waveforms zoom (⌘-wheel) down to beat level.
